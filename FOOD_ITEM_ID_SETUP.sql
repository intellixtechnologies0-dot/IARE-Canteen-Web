-- FOOD ITEM ID SETUP: Generate unique food_item_id and link orders to food items
-- This script creates unique IDs for food items and links orders to them
...
-- Step 4: Link orders to food items by matching item names
UPDATE public.orders 
SET food_item_id = (
    SELECT fi.food_item_id 
    FROM public.food_items fi 
    WHERE (fi.name = orders.item_name OR fi.item_name = orders.item_name)
    LIMIT 1
)
WHERE orders.food_item_id IS NULL;

-- Step 5: Update item_name in orders to show the actual food item name
UPDATE public.orders 
SET item_name = (
    SELECT fi.name 
    FROM public.food_items fi 
