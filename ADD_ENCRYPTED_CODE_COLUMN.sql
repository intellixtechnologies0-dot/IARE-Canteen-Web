-- Add Encrypted Code Column to Orders Table
-- Run this in your Supabase SQL Editor

-- 1. First, let's check what columns currently exist in the orders table
SELECT column_name, data_type, is_nullable
FROM information_schema.columns 
WHERE table_name = 'orders' AND table_schema = 'public'
ORDER BY ordinal_position;

-- 2. Add encrypted_code column to orders table
...
-- 4. Create function to generate unique encrypted code
...
BEGIN
    LOOP
        ...
        -- Generate encrypted code using MD5 hash
...
-- 5. Create trigger function to set encrypted_code on insert
...
-- 6. Create trigger to automatically set encrypted_code
...
-- 8. Update existing orders that don't have encrypted codes
UPDATE public.orders 
SET encrypted_code = public.generate_encrypted_code()
WHERE encrypted_code IS NULL;

-- 9. Create index for faster encrypted code lookups
...
-- 11. Show sample encrypted codes
SELECT 
    id,
    item_name,
    order_token,
